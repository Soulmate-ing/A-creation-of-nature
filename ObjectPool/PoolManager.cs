...
public class PoolManager : MonoBehaviour
{
    ...
    public void OnParticleEffectEvent(ParticleEffectType effectType, Vector3 pos)
    {
        //WORKFLOW:根据特效补全
        ObjectPool<GameObject> objPool = effectType switch
        {
            ParticleEffectType.LeavesFalling => poolEffectList[0],
    ...
    {
        var parent = new GameObject(poolPrefabs[2].name).transform;
        parent.SetParent(transform);

        for (int i = 0; i < 20; i++)
        {
            GameObject newObj = Instantiate(poolPrefabs[2], parent);
